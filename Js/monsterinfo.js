...
let quillAdd, quillEdit; // ตัวแปรเก็บ Instance ของ Quill Editor
...
function displayContent(page) {
    ...
    if (interactionSection) {
        ...
        const commentCount = page.monster_comments && page.monster_comments[0] ? page.monster_comments[0].count : 0;
...
async function loadWikiComments() {
    ...
    const { data: comments, error } = await supabaseClient
        ...
        .select('*')
        .eq('monster_page_id', activePageId)
...
async function submitWikiComment() {
    ...
    const { error } = await supabaseClient.from('monster_comments').insert([{ 
        monster_page_id: activePageId,
...
function openAddModal() {
    ...
    // เคลียร์เนื้อหาใน Quill
    if (quillAdd) quillAdd.root.innerHTML = '';
...
function openEditModal() {
    ...
    // นำเนื้อหาเก่าไปใส่ใน Quill Editor
    if (quillEdit) {
        // ถ้าเป็นข้อความธรรมดา จะถูกห่อหุ้มอัตโนมัติ ถ้าเป็น HTML ก็จะแสดงปกติ
...
// ==========================================
// ส่วนที่ 7: ตั้งค่า Quill Rich Text Editor
// ==========================================
function initQuillEditors() {
    const toolbarOptions = [
      [{ 'align': [] }], // ซ้าย กลาง ขวา
      [{ 'color': [] }, { 'background': [] }], // สีตัวอักษร และ สีพื้นหลังข้อความ
      ['bold', 'italic', 'underline', 'strike'], // ตัวอักษรพื้นฐาน
      ['image'], // อัปโหลดรูปภาพ
      ['clean'] // ลบ Format
    ];

    quillAdd = new Quill('#addEditorContainer', {
      theme: 'snow',
      modules: {
        toolbar: {
            container: toolbarOptions,
            handlers: {
                image: () => uploadImageToSupabase(quillAdd)
            }
        },
        imageResize: {
            displaySize: true // แสดงขนาดกว้างxยาวตอนปรับรูป
        }
      }
    });

    quillEdit = new Quill('#editEditorContainer', {
      theme: 'snow',
      modules: {
        toolbar: {
            container: toolbarOptions,
            handlers: {
                image: () => uploadImageToSupabase(quillEdit)
            }
        },
        imageResize: {
            displaySize: true
...
function uploadImageToSupabase(quillInstance) {
    ...
    input.onchange = async () => {
        ...
        if (!allowedTypes.includes(file.type)) {
            showAlert('ประเภทไฟล์ไม่รองรับ', 'รองรับเฉพาะ JPG, PNG, WebP, GIF เท่านั้น');
            return;
        }

        showAlert('กำลังอัปโหลด...', 'กำลังบีบอัดและอัปโหลดรูปภาพ...');

        // บีบอัดและปรับขนาดรูปด้วย Canvas
        const compressedFile = await compressImage(file, 800, 600, 0.75);

        const fileExt = 'jpg'; // บีบอัดเป็น jpg เสมอ
